...
export default function test() {
	...
	const filteredCartItems: Array<any> = cartitems.filter(
							...
							<Dialog.Panel className="relative transform overflow-hidden rounded-3xl bg-white p-10 text-left shadow-xl transition-all h-[1400px] w-[800px] border-8 border-primary">
									...
									<div className="mt-3 text-center sm:mt-5">
										...
										<div className="mt-2">
											<p className="text-2xl text-gray-500 pb-20">
												Voici un récapitulaif de votre
												commande :
											</p>
										</div>
									</div>
									...
									{filteredCartItems
										...
										.map((item: any, index: number) => (
											<div className="mb-2">
													...
													<div className="flex gap-5">
														...
														<div className="flex gap-1 bg-gray-200 p-1 rounded-full h-8">
															<button
																...
																className="cursor-pointer w-5 h-5 flex items-center justify-center text-2xl font-semibold text-black">
																-
															</button>
															<span className="w-5 h-5 flex items-center justify-center font-semibold text-black text-xl">
																{item.quantity}
															</span>
															<button
																onClick={() =>
												...
												<div
													className="flex justify-between gap-5 px-5 py-2 text-xl"
													key={index}>
								...
								<div className="w-full flex items-center justify-center gap-5 py-3 pt-10">
									<button
										...
										className="uppercase text-xl font-semibold w-96 py-3 px-2 bg-gray-200 rounded-full">
										Retour à ma commande
									</button>
									<button
										onClick={() => handleClick()}
										className="uppercase text-xl font-semibold w-96 py-3 px-2 bg-primary text-white rounded-full">
										Passer au paiement
									</button>
								</div>
							</Dialog.Panel>
