...
export default function test() {
	...
	const checkForPayment = async () => {
		const response = await fetch(
			"http://localhost:3000/api/get-payment-info",
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
				...
				body: JSON.stringify({
					PaymentIntentId: PaymentIntentdata.paymentIntent.id,
				}),
			}
		);
	...
	const simulatePayment = async () => {
		const response = await fetch(
			"http://localhost:3000/readers/simulate-payments",
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
	...
	return (
		<Transition.Root
			show={paymentBorne === true && paymentCompleted === false}
			as={Fragment}>
			<Dialog
				as="div"
