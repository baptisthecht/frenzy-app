...
export default function test() {
	...
	const paymentBorne = useSelector(
		(state: any) => state.payment.paymentBorne
	);
	...
	const command_number = useSelector(
		(state: any) => state.payment.command_number
	);
							...
							{command_number !== undefined ? (
								<Dialog.Panel className="relative transform overflow-hidden rounded-3xl bg-white p-10 text-left shadow-xl transition-all h-[1400px] w-[800px] border-8 border-primary">
									...
									<div className="flex flex-col items-center h-[900px] justify-center overflow-y-scroll scroll-gray">
										...
										<span className="text-[10rem] font-bold text-primary/70">
											{command_number < 10 ? "0" : ""}
											{command_number < 100 ? "0" : ""}
											{command_number}
										</span>
									</div>
									...
									<div className="flex flex-col w-full justify-center items-center">
										<span className="text-2xl">
											{paymentBorne === false
												? "Dirigez vous vers le comptoir pour régler votre commande"
												: ""}
											{bringsToTable === false ||
											(whereToEat == "À emporter" &&
												paymentBorne === true)
												? "Dirigez vous vers le comptoir pour récuperer votre commande"
												: ""}
											{bringsToTable === true &&
											paymentBorne === true &&
											whereToEat == "Sur place" &&
											hereChoice == "En salle"
												? "Installez vous en salle, on s'occupe de tout !"
												: ""}
											{bringsToTable === true &&
											paymentBorne === true &&
											whereToEat == "Sur place" &&
											hereChoice == "En terrasse"
												? "Installez vous en terrasse, on s'occupe de tout !"
												: ""}
										</span>
										<br />
										<span className="text-2xl">
								...
								<Dialog.Panel className="relative transform overflow-hidden rounded-3xl bg-white p-10 text-left shadow-xl transition-all h-[1400px] w-[800px] border-8 border-primary">
									<div>
										<div className="mt-3 text-center sm:mt-5">
											<Dialog.Title
												as="h3"
												className="font-semibold leading-6 text-gray-900 text-4xl pb-5"></Dialog.Title>
											<div className="mt-2">
												<p className="text-2xl text-gray-500 pb-20"></p>
											</div>
										</div>
									</div>
									<div className="flex flex-col items-center h-[900px] justify-center overflow-y-scroll scroll-gray">
