...
export default function Example(props: any) {
	...
	useEffect(() => {
		setTimeout(() => {
			if (
				actualVariants.actualVariants.length - 1 ===
									...
									<div className="mt-5 sm:mt-6 sm:grid sm:grid-flow-row-dense sm:grid-cols-2 sm:gap-3">
										{variant.options?.map((option: any) => (
											<button
												...
												key={option.name}>
												<span className="text-2xl flex flex-col gap-5 items-center">
													<img
														className="w-44"
														src={
															option.image
												...
												<span className="ml-1 text-md text-primary whitespace-nowrap">
													{option.supplement > 0
														? "(+ " +
														  option.supplement.toFixed(
																2
														  ) +
														  "€)"
														: ""}
